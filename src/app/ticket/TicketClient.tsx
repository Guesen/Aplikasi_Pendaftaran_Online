...
export default function TicketClient() {
  ...
  const patientName = useMemo(() => searchParams.get("name"), [searchParams]);
  const patientAddress = useMemo(() => searchParams.get("address"), [searchParams]);
  const patientPhone = useMemo(() => searchParams.get("phone"), [searchParams]);

  useEffect(() => {
    const generateTicket = async () => {
      if (!serviceName || !serviceType || !patientName || !patientAddress) {
        setError("Informasi layanan atau data pasien tidak valid. Silakan kembali ke halaman utama.");
        setIsLoading(false);
        return;
      }
      ...
      if (storedTicketJson) {
        ...
        // Also check if patient name is the same, in case of new registration
        if (storedTicket.serviceName === serviceName && storedTicket.patientName === patientName) {
  ...
  }, [serviceName, serviceType, patientName, patientAddress, patientPhone]);
    ...
    <>
      <div id="printable-ticket" className="flex justify-center p-4 bg-background print:bg-white">
        <div className="w-full max-w-xs font-mono bg-white text-black p-4 rounded-lg shadow-lg print:shadow-none print:p-0">
          ...
          <div className="border-t-2 border-dashed border-black pt-2 text-xs text-left space-y-1">
             <p><strong>Nama:</strong> {ticketData.patientName}</p>
             <p><strong>Alamat:</strong> {ticketData.patientAddress}</p>
             {ticketData.patientPhone && <p><strong>No. HP:</strong> {ticketData.patientPhone}</p>}
          </div>
